...
def determine_bot_functions(bot_names):
    ...
    for name in bot_names:
        if name == "student":
            ...
            bot_list.append(bots.TABot2())
        elif name == "ta3":
            bot_list.append(bots.TABot3())
        elif name == 'voronoi':
            bot_list.append(bots.VoronoiBot())
        elif name == 'gfong1':
            bot_list.append(bots.gfong1())
        else:
            raise ValueError('''Bot name %s is not supported. Value names include "student", 
"random", "wall", "tabot1", "tabot2" ''' % name)
    return bot_list
