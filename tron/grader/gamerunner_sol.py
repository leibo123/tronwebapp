...
def run_game(asp, bots, visualizer=None, delay=0.2, max_wait=0.3, colored=True):
    ...
    while not (asp.is_terminal_state(state)):
        ...
        signal.signal(signal.SIGALRM, support_sol.timeout_handler)
        signal.setitimer(signal.ITIMER_REAL, max_wait)
        try:
            # run AI
            decision = bots[state.ptm].decide(exposed)
            signal.setitimer(signal.ITIMER_REAL, 0)
        except support_sol.TimeoutException as msg:
            if visualizer:
                print(
...
def main():
    ...
    parser.add_argument(
        "-map",
        type=str,
        help='''the filename of the map to use for this game.
Defaults to "./maps/empty_room.txt"''',
        default="./maps/empty_room.txt",
    )
    parser.add_argument(
        "-max_wait",
        type=float,
        help="""The amount of time (in seconds) the game engine will wait
for a player to decide what move they want to make. If the player takes too long, they go north.
Defaults to 0.3 (this will be reset during grading)""",
        default=0.3,
    )
    parser.add_argument(
        "-bots",
        type=str,
        nargs="+",
        help="""which bot each player will use. Valid bot
...
the number of players on the board.""",
    ...
    parser.add_argument(
        "-image_delay",
        type=float,
        help="""The amount of time (in seconds) to wait after
...
Defaults to 0.2 seconds.""",
    ...
    parser.add_argument(
        "-no_image",
        dest="show_image",
        help="""include this flag (with no arguments) to supress the
output of all board states and just get final results""",
        action="store_false",
    )
    ...
    parser.add_argument(
        "-multi_test",
        type=int,
        help="""Test this map (multi_test) times in a row. Useful if you
...
how many times each player won across all games.""",
    ...
    wait = args.max_wait
    bots = support_sol.determine_bot_functions(args.bots)
    ...
    if multi is not None:
        ...
        for i in range(multi):
            ...
            outcome = run_game(game, bots, visualizer, delay, wait)
